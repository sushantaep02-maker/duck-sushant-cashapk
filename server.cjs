...
function extractAvatarFromUniversalJson(obj) {
  return extractFirstStringByKeys(
    obj,
    new Set(["avatarLarger", "avatarMedium", "avatarThumb"])
  );
}

// TikTok “display name” is typically stored as `nickname`
function extractDisplayNameFromUniversalJson(obj) {
  return extractFirstStringByKeys(obj, new Set(["nickname", "displayName"]));
}
...
app.get("/tiktok", async (req, res) => {
  ...
  try {
    ...
    // Best source: TikTok embedded JSON
    ...
    if (avatar) {
      ...
      const payload = {
        name: displayName || username, // ✅ display name if available
        username,                      // ✅ @username
        avatar: proxied
      };
    ...
    // Likely blocked/captcha/removed profile
    return res.json({
      name: username,
      username,
      avatar: null,
    ...
    return res
      .status(500)
      .json({ error: "TikTok fetch failed", details: String(e) });
  } finally {
